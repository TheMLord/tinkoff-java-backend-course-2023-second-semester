...
import static edu.java.bot.configuration.retry.BackOffPolicy.CONSTANT;
...
public class ConstantRetryTest {
    ...
    @Test
    @DisplayName(
        "Test that the scraper client makes a repeat request and does so the expected number of times and at the same interval")
    void testThatTheScraperClientMakesARepeatRequestAndDoesSoTheExpectedNumberOfTimesAndAtTheSameInterval() {
    ...
    private static void setScrapperProxyProperty(DynamicPropertyRegistry registry) {
        ...
        registry.add("app.retry.back-off-policy", () -> CONSTANT);
        registry.add("app.retry.httpStatuses", () -> HttpStatus.NOT_ACCEPTABLE);
        registry.add("app.retry.max-attempts", () -> 5);
